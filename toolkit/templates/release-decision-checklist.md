...
Os quatro estados são os do [contrato comum dos decision gates](../../docs/framework/08-implementation-and-adoption.md#decision-gates). `expired` não é uma disposição: é o estado que uma decisão `approve` ou `condition` assume quando ultrapassa o expiry sem revalidação, e exige nova decisão antes da continuidade.
