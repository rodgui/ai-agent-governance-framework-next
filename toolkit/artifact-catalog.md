...
| Authorized operational validation plan | prerequisites, test plan, acceptance criteria e cleanup para substitution/replay, observability e estate validation | governança + plataforma + Run Authority | W4/W5 | [execution package](assessments/authorized-operational-validation-plan.md) — `BLOCKED_BY_AUTHORIZED_EVIDENCE` |
| Dependency security triage | ecosystems, manifests, Dependabot strategy, alert-by-alert evidence e remediation record | repository maintainers + Security | W4/W5 | [assessment](assessments/dependency-security-triage.md) — `NOT_CONFIRMED` até export autorizado |
