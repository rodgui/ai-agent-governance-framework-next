...
### 3.2.1 Conflitos entre control planes

Quando uma decisão atravessa mais de um control plane, gateway ou sistema de authority, nenhum componente ganha precedência apenas por executar o routing. A decisão é composta pelos enforcement points obrigatórios do tier e da capability. Para identity, data, privacy, admissibility, security e ações state-changing, o `deny`, `restricted`, `quarantined` ou `missing` crítico prevalece até que uma authority compatível registre a exceção ou a remediação.

A divergência entre systems of record, owners, tiers, scopes, lifecycle ou policy status é um finding. Não deve ser resolvida silenciosamente por timestamp, cache ou conveniência do orchestrator. O fluxo registra `correlation_id`, authorities consultadas, decisões locais, ponto de conflito, estado fail-safe, evidence references e próximo owner. Consulte a [ADR-0015 — Arbitragem entre múltiplos control planes](../architecture/decisions/0015-multi-control-plane-arbitration.md) e o pattern [Multi-Control-Plane Governance](../../toolkit/patterns/multi-control-plane-governance.md).

**Concluído quando:** um caso atravessando dois planos identifica authority primária e alternativa, produz uma decisão determinística, preserva o conflito como finding e alcança contenção ou resolução sem handoff órfão.
