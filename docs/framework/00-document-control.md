...
| R3 | Declarar metadata suficiente para distinguir estado documental/editorial, disposição da decisão quando aplicável, maturidade/evidence e tipo de artefato, além do que cada estado permite | decisão de aprovação, aprovador, data, condições, efetividade, evidência de adoção e aliases históricos identificados como migration context | nenhum draft, caso ou fonte histórica é confundido com requisito vigente |
